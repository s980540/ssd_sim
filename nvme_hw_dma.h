...
typedef struct _hw_dma_dw0_t
{
    u32 next : 16;
    u32 rsvd : 16;
} hw_dma_dw0_t;

typedef struct _hw_dma_ctl_t
{
    u32 dspec : 16; // directive specific
    u32 rsvd : 16;
} hw_dma_ctl_t;

typedef struct _hw_dma_nlb_t
{
...
typedef struct _hw_dma_lba_max_t
{
    u32 desc_id : 4;        //
...
typedef struct _hw_dma_lba_low_t
{
    u32 lba_31_0 : 32;  // LBA bit[31:0]
} hw_dma_lba_low_t;

typedef struct _hw_dma_nsp_t
{
    u32 namespace_id : 32;  // namespace
} hw_dma_nsp_t;

typedef struct _hw_dma_desc_t
{
    hw_dma_dw0_t dw0;
    hw_dma_ctl_t ctl;
    hw_dma_nlb_t nlb;
    hw_dma_lba_max_t lba_max;

    hw_dma_buf_t buf;
    hw_dma_uid_t uid;
    hw_dma_sts_t sts;
    hw_dma_glb_t glb;
    hw_dma_lba_low_t lba_low;
    hw_dma_nsp_t nsp;
    u32 dw10;
} hw_dma_desc_t;
